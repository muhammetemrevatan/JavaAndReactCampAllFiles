				...
				if(user.getLastname().length() >= 2) {
					if(emailCheckService.emailCheck(user) == true) {
						if(emailIsItUsed(user.getEmail()) == true) {
							...
							System.out.println("Kayýt baþarýsýz. Bu eposta daha önce alýnmýþ..");
						}
					}else {
	...
	public void login(User user) {
		...
		for(String i : userDao.getEmail()) {
			if(i.equals(id)) {
				...
				System.out.println("Kullanýcý sistemde kayýtlý deðil. Giriþ baþarýsýz.");
			}
		}
		if(result) {
